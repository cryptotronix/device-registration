...
public class DeviceRegistrationUtilities {
	...
	public static ECKey generateNewJWK() throws NoSuchAlgorithmException,
			NoSuchProviderException, InvalidAlgorithmParameterException {
		KeyPair pair = generateKeyPair();

		return toPrivateJWK((ECPublicKey) pair.getPublic(),
				(ECPrivateKey) pair.getPrivate());

	}

	public static ECKey toPublicJWK(ECPublicKey key) {
		return new ECKey(ECKey.Curve.P_256, key, KeyUse.SIGNATURE, null,
				JWSAlgorithm.ES256, null, null, null, null);
	}

	public static ECKey toPrivateJWK(ECPublicKey pubKey, ECPrivateKey privKey) {
		return new ECKey(ECKey.Curve.P_256, pubKey, privKey, KeyUse.SIGNATURE,
				null, JWSAlgorithm.ES256, String.valueOf(System
						.currentTimeMillis()), null, null, null);

	}

	public static JWTClaimsSet buildRegClaims(RegisteredDevice info, ECKey pubKey)
			throws IOException {
	...
	public static JWTClaimsSet buildDataClaims(RegisteredDevice info, ECKey pubKey,
	...
	public static SignedJWT createRegistrationData(RegisteredDevice info, ECKey jwk)
			throws JOSEException, IOException, NoSuchAlgorithmException,
	...
	public static SignedJWT createSignedData(RegisteredDevice info, ECKey jwk, String data)
			throws JOSEException, IOException, NoSuchAlgorithmException,
			InvalidKeySpecException {
