...
public class DeviceSoft extends Device  {
	...
	protected void writeRegistrationData(File f_pub, File f_pri, File f_jwt)
			throws IOException {

		try {
			this.privateKey = DeviceRegistrationUtilities.generateNewJWK();

			this.jwt = DeviceRegistrationUtilities.createRegistrationData(this,
					this.privateKey);

			BufferedWriter w_pub = new BufferedWriter(new OutputStreamWriter(
					new FileOutputStream(f_pub)));
			BufferedWriter w_pri = new BufferedWriter(new OutputStreamWriter(
					new FileOutputStream(f_pri)));
			BufferedWriter w_jwt = new BufferedWriter(new OutputStreamWriter(
					new FileOutputStream(f_jwt)));
	...
	protected void loadInfo() throws IOException {
		...
		InputStream in = getClass().getResourceAsStream("device.properties");
		...
		if (f_pub.createNewFile() || f_pri.createNewFile()
				|| f_jwt.createNewFile()) {
			...
			try {
				this.privateKey = ECKey.parse(DeviceRegistrationUtilities
						.loadFileAsString(f_pri));
				this.jwt = SignedJWT.parse(DeviceRegistrationUtilities
						.loadFileAsString(f_jwt));
				this.pub = this.privateKey.toECPublicKey();
	...
	public SignedJWT toSignedJWT(String data) throws IOException {
		try {
			return DeviceRegistrationUtilities.createSignedData(this,
					this.privateKey, data);
		} catch (NoSuchAlgorithmException | InvalidKeySpecException
				| JOSEException | IOException e) {
			throw new IOException(e);
		}
	}
